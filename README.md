...
The interior cavity is now also shaped to roughly match the item, not just sized to it - the project's namesake "cutout" feature, previously unimplemented (every bin's cavity was a plain rectangular box no matter what was going in it, regardless of proposal type). `backend/app/services/shape.py` guesses "box" / "rounded_box" / "cylinder" from the item's name via a keyword list, and `backend/app/services/stl.py`'s `_make_cavity` cuts a correspondingly shaped cavity (filleted corners for rounded_box, a round hole for cylinder). This is a real, if rough, improvement over "always a flat box" - it is **not** a true custom fit. A reference tool ([gridfinitygenerator.com/en/cutout](https://gridfinitygenerator.com/en/cutout)) builds pixel-perfect cutouts by importing an STL of the item and boolean-subtracting it; that's not viable as this project's default path because most real-world items (the kind `/identify` resolves from a product listing) don't have a public 3D model to import. STL import as an *additional* option for items that do have one, and/or real photo-based silhouette extraction via `/identify-image`, remain open roadmap items for a tighter fit than the keyword heuristic gives.

Known caveats: `/identify-image` is still a placeholder (it doesn't actually analyze the photo yet - image-based recognition is the roadmap item below). Wikidata dimensions are stored as plain numbers without units, so converting them to mm uses a size heuristic rather than a guaranteed-correct unit; this is documented in `backend/app/services/dimensions/wikidata.py`. The frontend hasn't been changed as part of this - it talks to whichever backend Docker Compose points it at. Magnet/screw holes are still 4 total per bin (inset from each outer corner) rather than one set per grid cell, which is a simplification for multi-cell bins versus the full Gridfinity spec. The cutout shape heuristic is a keyword guess against the item's name, not real shape recognition - an item whose name doesn't match any keyword (or is missing) always keeps the original plain rectangular cavity, and a cylindrical item in a non-square footprint gets a cavity sized to its *shorter* axis rather than an ellipse.
...
- [x] Cutout cavity shaped to the item (keyword-based box/rounded_box/cylinder heuristic)
- [ ] Tighter cutout fit: optional STL import (boolean-subtract, like gridfinitygenerator.com/en/cutout) and/or real photo-based silhouette extraction
